...
<x-layout.dashboard :dictionary="$dictionary" :concepts="$concepts">
    ...
    <div class="w-full overflow-hidden">
        <div class="flex items-center justify-between gap-2 mt-2">
            <p class="text-2xl text-gray-800 dark:text-gray-200">{{__('dashboard.attachments.header', ['concept' => $concept->name])}}</p>
            @can('must-be-owner', $dictionary)
            <a href="{{route('attachment.create', [$dictionary, $concept])}}" class="px-3 py-2 text-sm font-medium text-center text-blue-500 transition-all border border-blue-500 rounded-lg hover:text-white hover:bg-blue-600 focus:ring-4 focus:outline-none focus:ring-blue-300 dark:border-blue-700 dark:text-blue-700 dark:hover:text-white dark:hover:bg-blue-800 dark:focus:ring-blue-800">
                <svg class="inline-block w-4 h-4 sm:w-5 sm:h-5" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5 sm:w-6 sm:h-6">
