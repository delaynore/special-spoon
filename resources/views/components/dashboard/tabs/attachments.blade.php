...
<x-layout.dashboard :dictionary="$dictionary" :concepts="$concepts">
    ...
    <div class="w-full overflow-hidden">
        <div class="mt-2 flex items-center justify-between gap-2">
            ...
            <a href="{{route('attachment.create', [$dictionary, $concept])}}" class="w-fit flex items-center gap-1 cursor-pointer justify-center px-4 py-2 text-sm font-medium text-white rounded-lg bg-primary-700 hover:bg-primary-800 focus:ring-4 focus:ring-primary-300 dark:bg-primary-600 dark:hover:bg-primary-700 focus:outline-none dark:focus:ring-primary-800">
        ...
        <div class="attachments mt-3 grid grid-cols-2 md:grid-cols-3 xl:grid-cols-6 gap-3">
            ...
            <div class="max-w-xs overflow-hidden bg-white rounded-lg shadow-lg dark:bg-gray-800 dark:border dark:border-gray-500">
                ...
                <img class="w-full h-48 mt-2 object-contain" src="{{asset('storage/'.$attach->path)}}" alt="{{ $attach->name }}">

                <div class="flex items-center justify-between px-4 py-2 gap-2 bg-gray-400 dark:bg-gray-900">
                    <a target="_blank" href="{{asset('storage/'.$attach->path)}}" class="px-2 py-1 text-xs font-semibold text-gray-900 dark:bg-gray-800 dark:border dark:border-gray-200 dark:text-gray-200 uppercase transition-colors duration-300 dark:hover:bg-gray-700 dark:hover:text-white transform bg-white rounded hover:bg-gray-200 focus:bg-gray-400 focus:outline-none">{{ __('shared.open') }}</a>
                    @can('must-be-owner', $dictionary)
                    <div class="flex justify-center">
                        <button id="delete-{{$attach->name}}" data-modal-target="delete-attach-{{$attach->id}}" data-modal-toggle="delete-attach-{{$attach->id}}" class="px-2 py-1 text-xs font-semibold text-gray-900 uppercase transition-colors duration-300 dark:hover:bg-gray-700 dark:hover:text-white dark:bg-gray-800 dark:border dark:border-gray-200 dark:text-gray-200 transform bg-white rounded hover:bg-gray-200 focus:bg-gray-400 focus:outline-none" type="button">
                            {{ __('shared.delete')}}
                        </button>
                    </div>
                    <!--  -->
                    <div id="delete-attach-{{$attach->id}}" tabindex="-1" aria-hidden="true" class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-modal md:h-full">
                        <div class="relative p-4 w-full max-w-md h-full md:h-auto">
                            <div class="relative p-4 text-center bg-white rounded-lg shadow dark:bg-gray-800 sm:p-5">
                                ...
                                <div class="flex justify-center items-center space-x-4">
                                    <button data-modal-toggle="delete-attach-{{$attach->id}}" type="button" class="py-2 px-3 text-sm font-medium text-gray-500 bg-white rounded-lg border border-gray-200 hover:bg-gray-100 focus:ring-4 focus:outline-none focus:ring-primary-300 hover:text-gray-900 focus:z-10 dark:bg-gray-700 dark:text-gray-300 dark:border-gray-500 dark:hover:text-white dark:hover:bg-gray-600 dark:focus:ring-gray-600">
