...
@php
    ...
    $nextPage = $page + 1;
    $pagingItems = [];
    array_push($pagingItems, 1);
    if ($page > 3) {
        if ($page < $countPages - 2) {
            array_push($pagingItems, '...', $previousPage, $page, $nextPage, '...', $countPages);
        } else if ($page === $countPages) {
            array_push($pagingItems, '...', $previousPage, $page);
        } else if ($page === $countPages - 1) {
            array_push($pagingItems, '...', $previousPage, $page, $nextPage);
        ...
        array_push($pagingItems, $page, $nextPage, '...', $countPages);
    } else {
        array_push($pagingItems, $nextPage, '...', $countPages);
    }
    $attributes = \App\Models\Attribute::join('concept_attributes', 'attributes.id', '=', 'concept_attributes.fk_attribute_id')->where('concept_attributes.fk_concept_id', $concept->id)
...
<div class="overflow-x-auto overflow-y-scroll shadow-md">
    <table class="w-full text-sm text-center text-gray-500 dark:text-gray-400">
        <caption
            class="px-6 py-2 text-lg font-semibold text-left text-gray-800 bg-white dark:text-gray-200 dark:bg-gray-800">
        ...
        <tbody>
            @foreach ($array_examples as $k => $v)
                <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                    ...
                    @can('must-be-owner', $concept->dictionary)
                        <td class="px-2 py-2">
                            <a href="{{route('concept.example.edit', [$concept, 'exampleNumber' => $k])}}"
                                class="font-medium text-blue-600 dark:text-blue-500 hover:underline">{{ __('shared.edit')}}</a>
                        </td>
                        <td class="px-2 py-2">
                            <form action="{{route('concept.example.destroy', [$concept, 'exampleNumber' => $k])}}"
                                method="post">
                                @csrf
                                @method('delete')
                                <button type="submit"
                                    class="font-medium text-red-600 dark:text-red-500 hover:underline">{{ __('shared.delete')}}</button>
                            </form>
                        </td>
                    @endcan
                </tr>
            ...
            <div class="flex justify-between flex-1 sm:hidden">
                ...
                @else
                        @php
                            $previousPage = $page - 1;
                        @endphp
                        <a href="{{ route('concept.examples', ['dictionary' => $dictionary->id, 'concept' => $concept->id, 'page' => $previousPage]) }}"
            ...
            <div class="hidden sm:flex-1 sm:flex sm:items-center sm:justify-center">
                ...
                <div>
                    <span class="relative z-0 inline-flex rounded-md shadow-sm rtl:flex-row-reverse">
                        ...
                        @foreach ($pagingItems as $link)
                            @if ($link === '...')
                                <span aria-disabled="true">
                                    <span
                                        class="relative inline-flex items-center px-2 py-2 -ml-px text-sm font-medium leading-5 text-gray-700 bg-white border border-gray-300 cursor-default dark:bg-gray-800 dark:border-gray-600">{{ "..." }}</span>
                                </span>
                            @else

                                <a href="{{ route('concept.examples', ['dictionary' => $dictionary->id, 'concept' => $concept->id, 'page' => $link]) }}"
                                    class="{{$link === $page ? 'underline' : ''}} relative inline-flex items-center px-2 py-2 text-sm font-medium leading-5 text-gray-500 transition duration-150 ease-in-out bg-white border border-gray-300 hover:text-gray-400 focus:z-10 focus:outline-none focus:ring ring-gray-300 focus:border-blue-300 active:bg-gray-100 active:text-gray-500 dark:bg-gray-800 dark:border-gray-600 dark:active:bg-gray-700 dark:focus:border-blue-800">
                                    {{ $link }}
                                </a>
                            @endif
                        ...
                        {{-- Next Page Link --}}
                        @if ($page < $countPages) <a
                                href="{{ route('concept.examples', ['dictionary' => $dictionary->id, 'concept' => $concept->id, 'page' => $nextPage]) }}"
                                rel="next"
