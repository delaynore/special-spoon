...
<li data-el="concept" data-id="concept-{{$concept->id}}" class="cursor-pointer"
    data-owner="{{$dictionary->fk_user_id === auth()->user()->id ? 'true' : 'false'}}" data-name="{{$concept->name}}"
    ...
    <div class="{{request('concept') === $concept->id ? 'opened-concept' : ''}} lidiv mb-[1px] inline-flex items-center justify-between w-full transition-transform delay-100 border-r border-gray-100 dark:border-gray-900 hover:bg-gray-200 dark:hover:bg-gray-700 toggle">
        <div data-open-concept-route="{{route('concept.show', [$dictionary, $concept])}}" class="inline-flex items-start flex-grow text-left open-concept">
            ...
            <div class="py-1 text-gray-400 -ml-[2px] mr-1">-</div>
            @endif
            <span class="p-1 overflow-hidden text-gray-900 select-text dark:text-gray-200">{{$concept->name}}</span>
        </div>
        @if ($children->count() > 0)
            <div class="pl-[10px] inline-flex items-center pr-1 toggler hover:text-green-600 dark:text-gray-200 dark:hover:text-emerald-500">
