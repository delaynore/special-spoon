...
<div id="dropdown{{$concept->id}}" class="z-10 hidden bg-white divide-y divide-gray-100 rounded-lg shadow w-44 dark:bg-gray-700">
    ...
    <div class="py-1 text-gray-700 dark:text-gray-200">
        <a href="{{route('concept.create', ['dictionary' => $dictionary, 'parentId' => $concept->id])}}" class="flex items-center justify-between px-4 py-2 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white">{{__('Добавить сына')}}
        ...
        <a href="{{route('concept.create', ['dictionary' => $dictionary, 'parentId' => $concept->fk_parent_concept_id])}}" class="flex items-center justify-between px-4 py-2 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white">{{__('Добавить брата')}}
            <svg class="w-4 h-4" data-slot="icon" aria-hidden="true" fill="none" stroke-width="2" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
    ...
    <ul class="py-1 text-sm text-gray-700 dark:text-gray-200" aria-labelledby="-button">
        ...
        @if(Auth::check() && Auth::user()->id === $dictionary->user->id)
        <li>
            <a href="{{route('concept.edit', [$dictionary, $concept])}}" class="flex items-center justify-between px-4 py-2 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white">Редактировать
                <svg class="w-4 h-4" data-slot="icon" aria-hidden="true" fill="none" stroke-width="2" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
