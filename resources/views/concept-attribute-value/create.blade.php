...
$conceptAttr = $concept->attributes()->orderBy('created_at')->get();
@endphp

<x-layout.main>
    <x-slot name="navigation"></x-slot>
    <x-slot:title>{{ __('Создание экземпляра') }}</x-slot:title>
            ...
            <div class="relative p-4 bg-white rounded-lg shadow dark:bg-gray-800 sm:p-5">
                <div class="flex items-center justify-between pb-4 mb-4 border-b rounded-t sm:mb-5 dark:border-gray-600">
                    <h3 class="text-lg font-semibold text-gray-900 dark:text-white">
                        {{ __('concept-attribute-value.create.header', ['concept' => $concept->name]) }}
                    </h3>
                ...
                <!-- Modal body -->
                <form method="post" action="{{route('concept.example.store', [$concept])}}">
                    @csrf
                    <div class="grid gap-4 mb-4">
                        ...
                        <div class="col-span-2">
                            ...
                            <input type="number" step="1" pattern="[0-9]+" min="{{PHP_INT_MIN}}" max="{{PHP_INT_MAX}}" name="{{$attr->id}}" id="{{$attr->id}}" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500" value="{{old($attr->id)}}">
